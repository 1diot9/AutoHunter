...
- finish: 挖掘结束时调用（found=挖到了 / no_vuln=确认无漏洞 / ip_banned=已尝试 enable_proxy_mode 切代理后代理仍不可达或无代理配置，确认 IP 被封，等待重测阶段代理复测）。
...
1. 确认注册入口：页面有"注册"/"Register"/"Sign Up"链接或 JS 里有 /register 等端点？纯 CAS/SSO 站点（只有统一认证，无注册链接）→ reg_status=not_registrable。
2. 有注册入口 → 实际尝试：填入测试数据，用 ddddocr 过图形验证码，提交注册，观察阻断点。
3. 分类报告（必须附带实际 HTTP 请求/响应证据）：
   - 唯一阻断是手机短信验证码或邮箱验证码 → reg_status=registrable_verification_needed，在 block_reason 写明是短信还是邮箱，在 what_user_needs_to_provide 写"完成注册后的账号密码"。
   - 邮箱域名限制/邀请码/人工审核/403关闭 → reg_status=not_registrable，在 block_reason 写具体限制。
   - 成功创建账号 → 不需要 needs_auth，直接用登录态继续深挖。
禁止仅凭页面外观臆断，必须实际访问注册页面/提交注册请求后再判断。auth_assessment 的 evidence_request 必须填你实际尝试的 HTTP 请求响应摘要。
...
# 遇到需要注册/登录才能继续的目标 → finish(needs_auth)
当攻击面集中在登录后区域且你无法自动完成登录时，先确认注册入口是否存在，再实际走一遍注册流程，基于 HTTP 证据判断：
1. 确认注册入口：页面有"注册"/"Register"/"Sign Up"链接或 JS 里有 /register 等端点？纯 CAS/SSO 站点（只有统一认证，无注册链接）→ reg_status=not_registrable。
2. 有注册入口 → 实际尝试：填入测试数据，用 ddddocr 过图形验证码，提交注册，观察阻断点。
3. 分类报告（必须附带实际 HTTP 请求/响应证据）：
   - 唯一阻断是手机短信验证码或邮箱验证码 → reg_status=registrable_verification_needed，在 block_reason 写明是短信还是邮箱。
   - 邮箱域名限制/邀请码/人工审核/403关闭 → reg_status=not_registrable。
   - 成功创建账号 → 不需要 needs_auth，直接用登录态继续深挖。
禁止仅凭页面外观臆断，必须实际访问注册页面/提交注册请求后再判断。
