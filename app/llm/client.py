...
class LLMClient:
        ...
        for _attempt in range(max_retries + 5):
            try:
                if self._messages_protocol:
                    # Anthropic Messages 协议要求 max_tokens；omit 时用大兜底值。
    ...
    def _build_messages_payload(
        ...
        if isinstance(extra_body, dict) and extra_body:
            enable = extra_body.get("enable_thinking")
            effort = str(extra_body.get("reasoning_effort") or "").lower()
            if enable is False or effort in {"none", "off", "0"}:
                payload["thinking"] = {"type": "disabled"}
            elif enable is True or effort in {"low", "medium", "high"}:
